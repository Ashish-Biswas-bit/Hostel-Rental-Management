...
class ShopRentalPanel(tk.Frame):
    ...
    def setup_payment_form(self):
        ...
        tk.Button(self.right_frame, text="Submit Payment", bg="#1abc9c", fg="white", font=("Arial", 11),
                  command=self.submit_payment, width=20).pack(pady=10)
        
        tk.Button(self.right_frame, text="📢 WhatsApp Reminder", bg="#25D366", fg="white", font=("Arial", 11),
                  command=self.send_whatsapp_reminder, width=20).pack(pady=(0, 10))


    def setup_add_shop_button(self):
        tk.Button(self.right_frame, text="➕ Add Flat", bg="#3498db", fg="white", font=("Arial", 11),
                  command=self.open_add_shop_form, width=20).pack(pady=(10, 5))
    ...
    def open_add_shop_form(self):
        ...
        fields = [("NID No", "shop_no"), ("Renter Name", "renter_name"),
                  ("Address", "address"), ("Phone", "phone"),
                  ("Monthly Rent", "monthly_rent"), ("Advance Paid", "advance_balance")]
        self.shop_entries = {}
        ...
        tk.Button(form, text="Save", bg="#2ecc71", fg="white", font=("Arial", 11),
                  command=self.save_shop).pack(pady=15)

        tk.Button(form, text="← Back to List", command=self.load_shop_list,
    ...
    def save_shop(self):
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO shops (shop_no, renter_name, phone, address, advance_balance, monthly_rent) VALUES (?, ?, ?, ?, ?, ?)",
                           (data['shop_no'], data['renter_name'], data['phone'], data['address'], advance_balance, monthly_rent))
            conn.commit()
            conn.close()
    ...
    def edit_selected_shop(self):
        ...
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT renter_name, address, phone, monthly_rent, advance_balance FROM shops WHERE shop_no=?", (shop_no,))
        ...
        def save_edit():
            ...
            try:
                ...
                cursor.execute("""
                    UPDATE shops 
                    SET renter_name=?, address=?, phone=?, monthly_rent=?, advance_balance=?
                    WHERE shop_no=?
                """, (
